...
model HP_AW_PEleSet
  "Air-water heat pump with PEleSet and COP based on TAmb, Pele and TSup"
  ...
  */
  ...
  Modelica.Blocks.Interfaces.RealInput PEleSet
    "Set point for electrical power, in W"
    annotation (Placement(transformation(extent={{-126,40},{-86,80}})));
  Modelica.Blocks.Sources.RealExpression realExpression(y=Q)
    annotation (Placement(transformation(extent={{2,-10},{-18,10}})));
equation
  PEle = PEleSet;
  PFuel = 0;

  connect(preQ.Q_flow, realExpression.y) annotation (Line(
      points={{-40,0},{-19,0}},
      color={0,0,127},
      smooth=Smooth.None));
