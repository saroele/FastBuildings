...
model HVAC_KK_TZon_Tes_SingleInput "Total HVAC model for KK with TES with one input"
  ...
  Zones.BaseClasses.Resistance resTes(r = rTes) "Resistance for heat losses to technical room" annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 0, origin = {12, -68})));
  TwoPort_PrescribedHeatFlow TES annotation(Placement(transformation(extent = {{10, -10}, {-10, 10}}, rotation = 0, origin = {12, -90})));
  Modelica.Blocks.Interfaces.RealInput QTes "Charging is positive" annotation(Placement(visible = true, transformation(origin = {104.422, -64.3407}, extent = {{12, -12}, {-12, 12}}, rotation = 0), iconTransformation(origin = {-99.578, -80.3407}, extent = {{-12, -12}, {12, 12}}, rotation = 0)));
equation
  ...
  connect(capTes.heaPor, resTes.heaPor_a) annotation(Line(points = {{-24, -68}, {2, -68}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(resTes.heaPor_b, resTecRoo.heaPor_a) annotation(Line(points = {{22, -68}, {28, -68}, {28, 40}, {-6, 40}, {-6, 60}, {-14, 60}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(TES.heaPor_a, capHea.heaPor) annotation(Line(points = {{22, -90}, {40, -90}, {40, 60}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(capTes.heaPor, TES.heaPor_b) annotation(Line(points = {{-24, -68}, {-14, -68}, {-14, -90}, {2, -90}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(QTes, TES.QSet) annotation(Line(points = {{104.422, -64.3407}, {54, -64.3407}, {54, -74}, {12, -74}, {12, -79.4}}, color = {0, 0, 127}, smooth = Smooth.None));
